...
class CfrNet(nn.Module):
    def __init__(self, in_dim, hidden_dim_rep, hidden_dim_hypo, config, use_ipm=True):
        ...
        self.lambda_uplift_ranking = 0.001 # mmd+Men: 0.01 / wass+men: 0.001 / wass+wom: 0.001
        self.lambda_outcome_ranking = 0.002 / (64**2) # Men: 0.001, matrix: 0.001 / (64**2)  // wass+wom: 0.005 / (64**2)
        self.balanced_MSE = False

        self.rep = Net(in_dim, in_dim, hidden_dim_rep, config)
        self.h1 = Net(in_dim, 1, hidden_dim_hypo, config) #for t = 1
        self.h0 = Net(in_dim, 1, hidden_dim_hypo, config) # for t = 0
    ...
    def calculate_loss(self, data, config):
        ...
        #print('outputs_h1.shape', outputs_h1.shape)
        #print('outputs_h0.shape', outputs_h0.shape)
        ...
        if self.split_h:
            ...
            output_h1_counterfactual = outputs_h[treatment_type.squeeze(), 1].unsqueeze(dim=-1)
            output_h0_counterfactual = outputs_h[(~treatment_type).squeeze(), 0].unsqueeze(dim=-1)
        #print('outputs_h1_counterfactual.shape', outputs_h1.shape)
        #print('outputs_h0_counterfactual.shape', outputs_h0.shape)            

        if self.balanced_MSE == False:
            ...
            # Calculate balanced prediction loss
            pred_loss = self.calculate_balanced_prediction_loss(
                (outputs_h1, outputs_h0),
                (treated_y, control_y),
                (treated_weights, control_weights),
                config            
    ...
    def calculate_balanced_prediction_loss(self, y_pred, y, weights, config):
        """
        Calculate the prediction loss.
        y_pred is a tuple (treated, control)
        Same holds for y and weights
        ...
        treated_y, control_y = y
        treated_weights, control_weights = weights
        self.balanced_criterion = BMCLoss(1.5, config)
        self.optim.add_param_group({'params': self.balanced_criterion.parameters(), 'lr':0.01, 'name': 'noise_sigma'})
        loss = (treated_weights * self.balanced_criterion(outputs_h1, treated_y)).mean()
        loss += (control_weights * self.balanced_criterion(outputs_h0, control_y)).mean()
