...
def uplift_ranking_loss(y_true, t_true, t_pred, y0_pred, y1_pred):
    ...
    tau_pred_t = tau_pred[t_true==1.0].unsqueeze(1)
    tau_pred_c = tau_pred[t_true==0.0].unsqueeze(1)
    softmax_tau_t = F.softmax(tau_pred_t, dim=0)
    softmax_tau_c = F.softmax(tau_pred_c, dim=0)
    #softmax_tau_t = F.softmax(torch.concat([tau_pred[t_true==1.0], tau_pred[t_true==0.0]], dim=0), dim=0)[:tau_pred[t_true==1.0].shape[0]]
    #softmax_tau_c = F.softmax(torch.concat([tau_pred[t_true==1.0], tau_pred[t_true==0.0]], dim=0), dim=0)[tau_pred[t_true==1.0].shape[0]:]
    treated_y = y_true[t_true==1.0].unsqueeze(1)
    control_y = y_true[t_true==0.0].unsqueeze(1)
    N1 = treated_y.shape[0]
    N0 = control_y.shape[0]
    loss = - (N1 + N0) * ((1/N1)*torch.sum(treated_y * torch.log(softmax_tau_t)) - (1/N0)*torch.sum(control_y * torch.log(softmax_tau_c)))
    return loss    
