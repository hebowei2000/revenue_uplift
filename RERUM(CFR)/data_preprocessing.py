...
def Histrom_Binary_Men(set):
    ...
    #dataset['true_spend'] = dataset['spend'] 
    #dataset['spend'] =  (-2 + 4 * dataset['CASE']) * dataset['spend']  
    return Histrom_Binary_Men_Dataset(dataset)
    
def Histrom_Binary_Wom(set):
    if set == 'train':
        dataset = pd.read_pickle(road + '/dt_binary_wom_tv_train.pkl')
        
    elif set == 'val':
        dataset = pd.read_pickle(road + '/dt_binary_wom_tv_val.pkl')
    
    elif set == 'test':
        dataset = pd.read_pickle(road + '/dt_binary_wom_test.pkl')

    #dataset['true_spend'] = dataset['spend'] 
    #dataset['spend'][dataset['CASE']==0] = dataset['spend'][dataset['CASE']==0] / 10
   # dataset['spend'] =  (-2 + 4 * dataset['CASE']) * dataset['spend']          
    return Histrom_Binary_Men_Dataset(dataset)
